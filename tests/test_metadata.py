...
def test_version_is_exported() -> None:
    ...
    assert re.fullmatch(r"\d+\.\d+\.\d+", shortfall.__version__)
...
def test_exported_version_matches_the_installed_distribution() -> None:
    ...
    version is a lie.
    """
    assert shortfall.__version__ == version("shortfall")
...
def test_exported_version_matches_the_source_of_truth() -> None:
    ...
    assert shortfall.__version__ == declared
...
def test_the_typing_marker_ships() -> None:
    ...
    """
    assert (Path(shortfall.__file__).parent / "py.typed").is_file()
    assert "Typing :: Typed" in metadata("shortfall").get_all("Classifier", [])


def test_the_entry_point_reports_the_version() -> None:
    """`shortfall --version` prints it, and exits zero doing so.

    argparse's version action raises SystemExit, so the assertion is on the
    exit code as much as on the text.
    """
    from shortfall.cli import main
